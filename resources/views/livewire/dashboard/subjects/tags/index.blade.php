<div>
    ...
    <div class="page-header d-print-none">
        <div class="container-xl">
            <div class="row g-2 align-items-center">
                <div class="col">
                    <h2 class="page-title">
                        Manage&nbsp;<a href="{{ route('dashboard.subjects.index') }}">{{ ucfirst($subject->name) }}</a>&nbsp;Tags
                    </h2>
                </div>
                <!-- Page title actions -->
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Subject {{ ucfirst($subject->name) }} Tag List</h3>
                        </div>
                                    ...
                                    <tr wire:key="{{ $tag->id }}">
                                        <td>
                                            <span class="text-secondary">
                                                {{ ($tags->currentpage()-1) * $tags->perpage() + $loop->index + 1 }}
                                            </span>
                                        </td>
                                        <td>
                                            {{ $tag->name  }}
                                                ...
                                                <span class="dropdown">
                                                  ...
                                                  <div class="dropdown-menu dropdown-menu-end">
                                                    <button class="dropdown-item"
                                                            wire:click="prepareItemEditing({{ $tag->id  }})">Edit</button>
                                                    <button class="dropdown-item"
                                                            wire:click="prepareItemDeletion({{ $tag->id }})">Delete</button>
                                                  </div>
                                                </span>
                                        </td>
                                    </tr>
                        ...
                        <div class="card-footer d-flex align-items-center">
                            <p class="m-0 text-secondary">
                                Showing
                                <span>{{  $tags->firstItem()  }}</span>
                                to
                                <span>{{ $tags->lastItem() }}</span>
                                of
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">New Subject Rate</h5>
                    ...
                    <form id="modal-create-form" wire:submit="store">
                        <div class="row mb-3">
                            <div class="col-12 mb-3 mb-md-0">
                                <div>
                                    <label for="number_of_students" class="form-label">Name</label>
                                    <input type="text" wire:model="form.name" class="form-control" id="name"
                                           placeholder="Ex. Unique Subject">
                                    @error('form.name')
    ...
    <div class="modal modal-blur fade" id="modal-edit" tabindex="-1" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Editing Subject {{ ucfirst($subject->name) }} Tag </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
